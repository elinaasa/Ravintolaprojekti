var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
// Function to fetch menu data from the server
function fetchMenus() {
    return __awaiter(this, void 0, void 0, function () {
        var response, data, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 3, , 4]);
                    return [4 /*yield*/, fetch('http://127.0.0.1:3000/api/menu')];
                case 1:
                    response = _a.sent();
                    if (!response.ok) {
                        throw new Error('Failed to fetch menus');
                    }
                    return [4 /*yield*/, response.json()];
                case 2:
                    data = _a.sent();
                    return [2 /*return*/, data.menus];
                case 3:
                    error_1 = _a.sent();
                    console.error('Error fetching menus:', error_1);
                    return [2 /*return*/, []];
                case 4: return [2 /*return*/];
            }
        });
    });
}
// function to update menu data on the server
function updateMenus(updatedMenus) {
    return __awaiter(this, void 0, void 0, function () {
        var response, error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, fetch('http://127.0.0.1:3000/api/menu', {
                            method: 'PUT',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                            body: JSON.stringify({ menus: updatedMenus }),
                        })];
                case 1:
                    response = _a.sent();
                    if (!response.ok) {
                        throw new Error('Failed to update menus');
                    }
                    return [3 /*break*/, 3];
                case 2:
                    error_2 = _a.sent();
                    console.error('Error updating menus:', error_2);
                    return [3 /*break*/, 3];
                case 3: return [2 /*return*/];
            }
        });
    });
}
// Frunction to display the menus
function displayMenus() {
    return __awaiter(this, void 0, void 0, function () {
        var menuList, existingMenus, addMenuItemButton, newMenuItemInput;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    menuList = document.getElementById('menuList');
                    return [4 /*yield*/, fetchMenus()];
                case 1:
                    existingMenus = _a.sent();
                    if (menuList) {
                        existingMenus.forEach(function (menu) {
                            var menuItemElement = document.createElement('div');
                            menuItemElement.textContent = menu.name;
                            menuList.appendChild(menuItemElement);
                        });
                    }
                    addMenuItemButton = document.getElementById('addMenuItem');
                    newMenuItemInput = document.getElementById('newMenuItem');
                    if (addMenuItemButton && newMenuItemInput) {
                        addMenuItemButton.addEventListener('click', function () {
                            var newMenuItemInput = document.getElementById('newMenuItem');
                            var newMenuItemName = newMenuItemInput.value;
                            var newMenu = {
                                id: existingMenus.length + 1,
                                name: newMenuItemName,
                                description: null,
                                diet: '',
                                week_number: 0,
                                day_of_week: 0,
                            };
                            existingMenus.push(newMenu);
                            var menuItemElement = document.createElement('div');
                            menuItemElement.textContent = newMenuItemName;
                            if (menuList) {
                                menuList.appendChild(menuItemElement);
                            }
                            newMenuItemInput.value = ''; // Clear input field after adding
                        });
                    }
                    return [2 /*return*/];
            }
        });
    });
}
;
// Function to save the menus
var saveChangesButton = document.getElementById('saveChanges');
if (saveChangesButton) {
    saveChangesButton.addEventListener('click', function () { return __awaiter(_this, void 0, void 0, function () {
        var existingMenus;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fetchMenus()];
                case 1:
                    existingMenus = _a.sent();
                    return [4 /*yield*/, updateMenus(existingMenus)];
                case 2:
                    _a.sent();
                    alert('Changes saved successfully!');
                    return [2 /*return*/];
            }
        });
    }); });
}
;
displayMenus();
