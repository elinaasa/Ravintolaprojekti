// export interface Course {
//   name: string;
//   description: string;
//   diet: string;
// }
var __awaiter =
  (this && this.__awaiter) ||
  function (thisArg, _arguments, P, generator) {
    function adopt(value) {
      return value instanceof P
        ? value
        : new P(function (resolve) {
            resolve(value);
          });
    }
    return new (P || (P = Promise))(function (resolve, reject) {
      function fulfilled(value) {
        try {
          step(generator.next(value));
        } catch (e) {
          reject(e);
        }
      }
      function rejected(value) {
        try {
          step(generator['throw'](value));
        } catch (e) {
          reject(e);
        }
      }
      function step(result) {
        result.done
          ? resolve(result.value)
          : adopt(result.value).then(fulfilled, rejected);
      }
      step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
  };
var __generator =
  (this && this.__generator) ||
  function (thisArg, body) {
    var _ = {
        label: 0,
        sent: function () {
          if (t[0] & 1) throw t[1];
          return t[1];
        },
        trys: [],
        ops: [],
      },
      f,
      y,
      t,
      g;
    return (
      (g = {next: verb(0), throw: verb(1), return: verb(2)}),
      typeof Symbol === 'function' &&
        (g[Symbol.iterator] = function () {
          return this;
        }),
      g
    );
    function verb(n) {
      return function (v) {
        return step([n, v]);
      };
    }
    function step(op) {
      if (f) throw new TypeError('Generator is already executing.');
      while ((g && ((g = 0), op[0] && (_ = 0)), _))
        try {
          if (
            ((f = 1),
            y &&
              (t =
                op[0] & 2
                  ? y['return']
                  : op[0]
                  ? y['throw'] || ((t = y['return']) && t.call(y), 0)
                  : y.next) &&
              !(t = t.call(y, op[1])).done)
          )
            return t;
          if (((y = 0), t)) op = [op[0] & 2, t.value];
          switch (op[0]) {
            case 0:
            case 1:
              t = op;
              break;
            case 4:
              _.label++;
              return {value: op[1], done: false};
            case 5:
              _.label++;
              y = op[1];
              op = [0];
              continue;
            case 7:
              op = _.ops.pop();
              _.trys.pop();
              continue;
            default:
              if (
                !((t = _.trys), (t = t.length > 0 && t[t.length - 1])) &&
                (op[0] === 6 || op[0] === 2)
              ) {
                _ = 0;
                continue;
              }
              if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                _.label = op[1];
                break;
              }
              if (op[0] === 6 && _.label < t[1]) {
                _.label = t[1];
                t = op;
                break;
              }
              if (t && _.label < t[2]) {
                _.label = t[2];
                _.ops.push(op);
                break;
              }
              if (t[2]) _.ops.pop();
              _.trys.pop();
              continue;
          }
          op = body.call(thisArg, _);
        } catch (e) {
          op = [6, e];
          y = 0;
        } finally {
          f = t = 0;
        }
      if (op[0] & 5) throw op[1];
      return {value: op[0] ? op[1] : void 0, done: true};
    }
  };

// Function to fetch menu data from the server
function fetchMenuData() {
  return __awaiter(this, void 0, void 0, function () {
    var response, data, error_1;
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          _a.trys.push([0, 3, , 4]);
          return [4 /*yield*/, fetch('/getMenus')];
        case 1:
          response = _a.sent();
          return [4 /*yield*/, response.json()];
        case 2:
          data = _a.sent();
          return [2 /*return*/, data];
        case 3:
          error_1 = _a.sent();
          console.error('Error fetching menu data:', error_1);
          return [2 /*return*/, null];
        case 4:
          return [2 /*return*/];
      }
    });
  });
}
// Function to print the weekly menu
function printWeeklyMenu(menuData) {
  var menuContainer = document.getElementById('menu-container');
  // Clear any existing content
  if (menuContainer) {
    menuContainer.innerHTML = '';
    // Loop through each week
    menuData.forEach(function (week) {
      // Create a heading for the week
      var weekHeading = document.createElement('h2');
      weekHeading.textContent = 'Week '.concat(week[0].week_number);
      menuContainer.appendChild(weekHeading);
      // Create a list for each day
      var weekList = document.createElement('ul');
      // Loop through each day
      week.forEach(function (menuItem) {
        // Create a list item for the menu item
        var listItem = document.createElement('li');
        listItem.textContent = ''
          .concat(getDayName(menuItem.day_of_week), ': ')
          .concat(menuItem.name);
        weekList.appendChild(listItem);
      });
      // Append the list to the container
      menuContainer.appendChild(weekList);
    });
  }
}
// Function to print the daily menu
function printDailyMenu(menuData, dayNumber) {
  // Assuming menuData is an array containing the menu items for the selected day
  // Get the element where you want to display the menu
  var menuContainer = document.getElementById('menu-container');
  // Clear any existing content
  if (menuContainer) {
    menuContainer.innerHTML = '';
    // Create a heading for the day
    var dayHeading = document.createElement('h2');
    dayHeading.textContent = ''.concat(getDayName(dayNumber), ' Menu');
    menuContainer.appendChild(dayHeading);
    // Create a list for the menu items
    var dayList_1 = document.createElement('ul');
    // Filter menuData for the selected day
    var dayMenu = menuData.filter(function (menuItem) {
      return menuItem.day_of_week === dayNumber;
    });
    // Loop through each menu item for the day
    dayMenu.forEach(function (menuItem) {
      // Create a list item for the menu item
      var listItem = document.createElement('li');
      listItem.textContent = menuItem.name;
      dayList_1.appendChild(listItem);
    });
    // Append the list to the container
    menuContainer.appendChild(dayList_1);
  }
}
// Helper function to get day name from day number
function getDayName(dayNumber) {
  var days = [
    'Sunday',
    'Monday',
    'Tuesday',
    'Wednesday',
    'Thursday',
    'Friday',
    'Saturday',
  ];
  return days[dayNumber - 1];
}
// Call the function to fetch menu data and print menus
fetchMenuData().then(function (menuData) {
  if (menuData) {
    // Example: Print weekly menu
    printWeeklyMenu(menuData);
    // Example: Print daily menu for Monday (day number 1)
    var selectedDay = 1;
    printDailyMenu(menuData.flat(), selectedDay);
  }
});
